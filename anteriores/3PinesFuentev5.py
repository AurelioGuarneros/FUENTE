...
def update_pwm():
    try:
        ...
        dead_time = 0.00001  # 10 microsegundos de tiempo muerto

        for pwm in pwms:
            pwm.ChangeFrequency(freq)
        
        # Asegurarse de que los pulsos no se encimen
        pwms[0].ChangeDutyCycle(duty1)  # Pin 18

        # Introduce un tiempo muerto
        time.sleep(dead_time)

        pwms[1].ChangeDutyCycle(100 - duty1)  # Pin 23 (inverso de pin 18)
        pwms[2].ChangeDutyCycle(duty3)  # Pin 24
    except ValueError:
