...
def pwm_thread():
    ...
    while running:
        # Calcula los tiempos de encendido y apagado para cada pin
        on_time_18 = (duty1 / 100.0) * max_duty_time
        off_time_18 = max_duty_time - on_time_18
        on_time_23 = max_duty_time - on_time_18  # Complementario de on_time_18

        # Pin 18 y Pin 23 alternados sin superposición
        GPIO.output(pwm_pins[0], GPIO.HIGH)
        GPIO.output(pwm_pins[1], GPIO.LOW)  # Asegura que el pin 23 está apagado
        time.sleep(on_time_18)
        GPIO.output(pwm_pins[0], GPIO.LOW)
        time.sleep(off_time_18)

        GPIO.output(pwm_pins[1], GPIO.HIGH)
        time.sleep(on_time_23)
...
def update_pwm():
    ...
    try:
        ...
        # Asegurar que el ciclo de trabajo esté entre 0 y 50%
        if duty1 > 50:
            duty1 = 50
        if duty3 > 100:
            duty3 = 100
